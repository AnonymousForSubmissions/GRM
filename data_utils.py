...
def evaluate_whole_graph(args, model, dataset_tr, dataset_val, datasets_te, eval_func, data_loaders=None, neighbor=False):
    ...
    accs, test_outs = [], []
    train_out, sample_idx = model.inference(dataset_tr)

    train_acc = eval_func(dataset_tr.label[sample_idx], train_out)
    #print(dataset_val)
    valid_out, sample_idx = model.inference(dataset_val)
    valid_acc = eval_func(dataset_val.label[sample_idx], valid_out)
    accs += [train_acc] + [valid_acc]
    for i, dataset in enumerate(datasets_te):

        out, sample_idx = model.inference(dataset)
        test_outs.append(out)
        accs.append(eval_func(dataset.label[sample_idx], out))

    return accs, test_outs


def evaluate_whole_graph_ogb(args, model, dataset_tr, dataset_val, datasets_te, eval_func, data_loaders=None, neighbor=False):
    model.eval()
...
def evaluate_whole_graph_multi(args, model, datasets_tr, datasets_val, datasets_te, eval_func):
    ...
    for i, dataset in enumerate(datasets_tr):
        out, sample_idx = model.inference(dataset)
        acc_tr += eval_func(dataset.label[sample_idx], out)
    accs += [acc_tr / len(datasets_tr)]
    for i, dataset in enumerate(datasets_val):
        out, sample_idx = model.inference(dataset)
        acc_val += eval_func(dataset.label[sample_idx], out)
    accs += [acc_val / len(datasets_val)]
    for i, dataset in enumerate(datasets_te):
        out, sample_idx = model.inference(dataset)
        test_outs.append(out)
        acc_te = eval_func(dataset.label[sample_idx], out)
        accs += [acc_te]
    return accs, test_outs


def evaluate_whole_graph_elliptic(args, model, datasets_tr, datasets_val, datasets_te, eval_func, data_loaders=None):
    model.eval()
    accs, test_outs = [], []
